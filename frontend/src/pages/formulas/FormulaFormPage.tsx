...
export default function FormulaFormPage() {
    ...
    if (isEdit) {
      getFormula(Number(id)).then((res) => {
        ...
        form.setFieldsValue({
          ...
          formulaType: f.formulaType ?? undefined,
  ...
  const handleIngredientSelect = (key: string, ingredientId: number) => {
    const ing = ingredients.find((i) => i.id === ingredientId)
    setRows((r) => r.map((row) => row.key === key
      ? { ...row, ingredientId, unit: ing?.unit ?? row.unit, unitPrice: ing?.unitPrice ?? null }
      : row
    ))
  }

  const handleSubmit = async (values: {
    code: string; name: string; category?: string; formulaType?: string
    productType: string; description: string; changeNote?: string
  }) => {
    ...
    try {
      ...
      if (isEdit) {
        await updateFormula(Number(id), {
          ...
          formulaType: values.formulaType ?? null,
  ...
  const columns = [
    {
      ...
      render: (_: unknown, record: IngredientRow) => (
        <Select
          ...
          onChange={(v) => handleIngredientSelect(record.key, v)}
          placeholder="選擇原料"
          showSearch
    ...
    },
    {
      ...
      render: (_: unknown, record: IngredientRow) => (
        <InputNumber
          style={{ width: '100%' }}
          min={0}
          value={record.unitPrice ?? undefined}
          onChange={(v) => updateRow(record.key, 'unitPrice', v)}
          placeholder="—"
        />
      ),
    },
  ...
  return (
    <Card title={isEdit ? '編輯配方' : '新增配方'}>
      <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ maxWidth: 860 }}>
        {!isEdit && (
        ...
        <Space style={{ width: '100%' }}>
          <Form.Item name="category" label="配方類別" style={{ flex: 1, minWidth: 160 }}>
            <DropdownSelect categoryKey="formula_category" allowClear placeholder="請選擇" style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="formulaType" label="配方類型" style={{ flex: 1, minWidth: 160 }}>
            <DropdownSelect categoryKey="formula_type" allowClear placeholder="請選擇" style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="productType" label="產品類型" rules={[{ required: true }]} style={{ flex: 1, minWidth: 160 }}>
            <Input />
          </Form.Item>
        </Space>
        ...
        <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>
          設定此配方所使用的原料及比例。選擇原料後單位與單價會自動帶入。
        </Typography.Text>
