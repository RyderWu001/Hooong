...
import {
  ...
  Space, Popconfirm, message, Select, DatePicker, Tag, Row, Col, Statistic, Descriptions, Alert,
} from 'antd'
import {
  PlusOutlined, EditOutlined, DeleteOutlined,
  WarningOutlined, FileProtectOutlined, AlertOutlined, BarChartOutlined,
...
import type { FormulaRisk, IngredientRisk, AbnormalEvent, RiskLevel, AbnormalEventStatus, Formula, Ingredient } from '../../types'
...
function FormulaRiskTab() {
  ...
  const handleSave = async (values: { formulaId: number; riskLevel: RiskLevel; riskType: string; description: string; mitigation: string; nextReviewAt: dayjs.Dayjs }) => {
    try {
      const payload = { ...values, nextReviewAt: values.nextReviewAt.toISOString() }
  ...
  const columns: ColumnsType<FormulaRisk> = [
    ...
    { title: '風險類型', dataIndex: 'riskType', key: 'riskType', width: 110 },
  ...
  return (
    <>
      ...
      <Modal open={modalOpen} title={editing ? '編輯配方風險' : '新增配方風險評估'} onCancel={() => setModalOpen(false)} onOk={() => form.submit()} destroyOnClose>
        <Form form={form} layout="vertical" onFinish={handleSave}>
          ...
          <Form.Item name="riskLevel" label="風險等級" rules={[{ required: true }]}><Select options={RISK_OPTIONS} /></Form.Item>
          <Form.Item name="riskType" label="風險類型" rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="description" label="風險描述" rules={[{ required: true }]}><Input.TextArea rows={3} /></Form.Item>
          <Form.Item name="mitigation" label="緩解措施" rules={[{ required: true }]}><Input.TextArea rows={3} /></Form.Item>
...
function IngredientRiskTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [rRes, iRes] = await Promise.all([getIngredientRisks({ riskLevel: filterLevel }), getIngredients({ limit: 200 })])
      setData(rRes.data.data); setIngredients(iRes.data.data)
    } finally { setLoading(false) }
  }, [filterLevel])
  ...
  const handleSave = async (values: Omit<IngredientRisk, 'id' | 'ingredientName' | 'assessedBy' | 'assessedAt'>) => {
    try {
      if (editing) { await updateIngredientRisk(editing.id, values); message.success('已更新') }
      else { await createIngredientRisk(values as Parameters<typeof createIngredientRisk>[0]); message.success('已新增') }
      setModalOpen(false); load()
    } catch { message.error('儲存失敗') }
  }
  ...
  return (
    <>
      <Space style={{ marginBottom: 16 }}>
        {canEdit && <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditing(null); form.resetFields(); setModalOpen(true) }}>新增風險評估</Button>}
        <Select allowClear placeholder="篩選風險等級" style={{ width: 150 }} options={RISK_OPTIONS}
          onChange={(v) => setFilterLevel(v as RiskLevel)} />
      </Space>
      <Table rowKey="id" loading={loading} columns={columns} dataSource={data} pagination={false} />

      <Modal open={modalOpen} title={editing ? '編輯原料風險' : '新增原料風險評估'} onCancel={() => setModalOpen(false)} onOk={() => form.submit()} destroyOnClose>
        <Form form={form} layout="vertical" onFinish={handleSave}>
          {!editing && (
          ...
          <Form.Item name="riskLevel" label="風險等級" rules={[{ required: true }]}><Select options={RISK_OPTIONS} /></Form.Item>
          <Form.Item name="riskType" label="風險類型" rules={[{ required: true }]}><Input /></Form.Item>
          <Form.Item name="hazardDescription" label="危害說明" rules={[{ required: true }]}><Input.TextArea rows={2} /></Form.Item>
          <Form.Item name="safeHandling" label="安全操作" rules={[{ required: true }]}><Input.TextArea rows={2} /></Form.Item>
          <Form.Item name="storageRequirements" label="儲存要求" rules={[{ required: true }]}><Input.TextArea rows={2} /></Form.Item>
        </Form>
      </Modal>

      <Modal open={!!detail} title="原料風險詳情" onCancel={() => setDetail(null)} footer={null}>
        {detail && (
          <Descriptions column={1} bordered size="small">
            <Descriptions.Item label="原料">{detail.ingredientName}</Descriptions.Item>
      ...
      <Modal open={addOpen} title="記錄異常事件" onCancel={() => setAddOpen(false)} onOk={() => addForm.submit()} destroyOnClose>
        <Form form={addForm} layout="vertical" onFinish={handleAdd} initialValues={{ occurredAt: dayjs() }}>
          ...
          <Form.Item name="eventType" label="事件類型" rules={[{ required: true }]}>
            <Select options={[{ value: '原料品質' }, { value: '實驗異常' }, { value: '庫存異常' }, { value: '設備故障' }, { value: '其他' }].map((o) => ({ value: o.value, label: o.value }))} />
          </Form.Item>
      ...
      <Modal open={!!detail} title="異常事件詳情" onCancel={() => setDetail(null)} footer={null} width={600}>
        {detail && (
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="關聯原料" span={2}>{detail.relatedIngredientName ?? '—'}</Descriptions.Item>
...
function RiskReportTab() {
  ...
  const highRiskItems = [
    ...
    ...Object.entries(report.ingRiskDist).filter(([k]) => k === 'HIGH' || k === 'CRITICAL').reduce((a, [, v]) => a + (v as number), 0) > 0
      ? [{ type: '原料高風險', count: (report.ingRiskDist.HIGH ?? 0) + (report.ingRiskDist.CRITICAL ?? 0) }]
      : [],
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col span={12}>
          <Card title="原料風險分布" size="small" loading={loading}>
            <Space direction="vertical" style={{ width: '100%' }}>
              {(Object.entries(RISK_LABEL) as [RiskLevel, string][]).map(([level, label]) => (
                <Space key={level}>
                  <Tag color={RISK_COLOR[level]} style={{ width: 60, textAlign: 'center' }}>{label}</Tag>
  ...
  return (
    <Card title="風險管理">
      <Tabs items={[
        ...
        { key: '2', label: '原料風險評估', icon: <WarningOutlined />, children: <IngredientRiskTab /> },
